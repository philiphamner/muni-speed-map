// Parse LA Metro GTFS data and extract Metro Rail lines as GeoJSON
import { readFileSync, writeFileSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';

const __dirname = dirname(fileURLToPath(import.meta.url));
const gtfsDir = join(__dirname, '..', 'gtfs_la', 'gtfs_rail-master');
const outputDir = join(__dirname, '..', 'src', 'data');

// LA Metro Rail lines (route_id from GTFS)
...
// Line colors matching LA Metro branding
const LINE_COLORS = {
  '801': '#0072BC', // A Line (Blue)
  '802': '#E4002B', // B Line (Red)
  '803': '#58A618', // C Line (Green)
  '804': '#FDB913', // E Line (Expo)
  '805': '#A05DA5', // D Line (Purple)
  '806': '#F9A51A', // L Line (Gold)
  '807': '#E96BB0', // K Line (Crenshaw)
};
...
function main() {
  console.log('Parsing LA Metro GTFS data...');
  ...
  console.log(`Found ${metroRoutes.length} metro rail routes:`, metroRoutes.map(r => `${r.route_id} (${r.route_short_name})`));
  
  // Parse trips to get shape_ids for each route
  const trips = parseCSV('trips.txt');
  console.log(`Total trips: ${trips.length}`);
  const metroTrips = trips.filter(t => METRO_LINES.includes(t.route_id));
  console.log(`Metro rail trips: ${metroTrips.length}`);
  ...
  // Write output
  const outputPath = join(outputDir, 'laMetroRoutes.json');
