// Parse SFMTA GTFS data and extract a lightweight bus overlay for SF testing.
...
const outputPath = join(__dirname, "..", "src", "data", "sfBusRoutesTest.json");
...
function main() {
  console.log("Parsing SFMTA GTFS bus routes...");
  ...
  const routeById = Object.fromEntries(
    busRoutes.map((route) => [route.route_id, route]),
  );

  console.log(`Bus routes (route_type=3): ${busRoutes.length}`);

  // Count trips by route+direction+shape to pick one representative shape per route direction.
  ...
  for (const trip of trips) {
    ...
    if (!candidatesByRouteDir[key]) {
      candidatesByRouteDir[key] = {};
    }
  ...
  Object.entries(candidatesByRouteDir).forEach(([routeDirKey, byShape]) => {
    ...
    selectedShapes[bestShapeId] = {
      ...bestInfo,
      route_dir_key: routeDirKey,
    };
  });

  console.log(
    `Representative shapes selected: ${Object.keys(selectedShapes).length}`,
  );
  ...
  const features = Object.entries(shapePoints)
    ...
    .filter(Boolean)
    .sort((a, b) => {
      if (a.properties.route_short_name !== b.properties.route_short_name) {
