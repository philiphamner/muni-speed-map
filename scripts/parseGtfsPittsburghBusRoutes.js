// Parse PRT GTFS data and extract a lightweight bus overlay for Pittsburgh.
import { readFileSync, writeFileSync } from "fs";
import { join, dirname } from "path";
import { fileURLToPath } from "url";

const __dirname = dirname(fileURLToPath(import.meta.url));
const gtfsDir = join(__dirname, "..", "gtfs_pittsburgh");
const outputPath = join(__dirname, "..", "src", "data", "pittsburghBusRoutesTest.json");
...
function main() {
  console.log("Parsing Pittsburgh GTFS bus routes...");
  ...
  for (const trip of trips) {
    ...
    if (!candidatesByRouteDir[key][trip.shape_id]) candidatesByRouteDir[key][trip.shape_id] = { count: 0, headsign: trip.trip_headsign || "", route_id: trip.route_id, direction_id: d };
