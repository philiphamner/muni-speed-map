// Parse Boston MBTA GTFS data and extract Green Line and Mattapan Trolley routes as GeoJSON
...
// MBTA Green Line branches + Mattapan Trolley (light rail / street-running)
...
// Mattapan = Mattapan Trolley (historic PCC cars, connects to Red Line)
const GREEN_LINE_ROUTES = ['Green-B', 'Green-C', 'Green-D', 'Green-E', 'Mattapan'];
...
function main() {
  ...
  const greenRoutes = routes.filter(r => GREEN_LINE_ROUTES.includes(r.route_id));
  console.log(`Found ${greenRoutes.length} Green Line routes:`, greenRoutes.map(r => `${r.route_id} (${r.route_long_name})`));
  
  // Parse trips to get shape_ids for each route
  const trips = parseCSV('trips.txt');
  console.log(`Total trips: ${trips.length}`);
  const greenTrips = trips.filter(t => GREEN_LINE_ROUTES.includes(t.route_id));
