#!/usr/bin/env node
/**
 * Parse Pittsburgh Regional Transit GTFS data to extract T Light Rail routes as GeoJSON
 * 
 * This script reads the GTFS files from gtfs_pittsburgh/ and generates:
 * - pittsburghTRoutes.json: Route geometries with per-line route_id and colors
 * - pittsburghTStops.json: Stop locations with route associations
 * 
 * Run with: node scripts/parseGtfsPittsburgh.js
 */
...
const GTFS_DIR = path.join(__dirname, "../gtfs_pittsburgh");
const OUTPUT_DIR = path.join(__dirname, "../src/data");

// Pittsburgh T Light Rail routes from PRT GTFS
// route_id -> { name, shortName, color }
// Order matters: first = bottom layer, last = top layer on map
const T_ROUTES = {
  "SLVR": { name: "Silver Line", shortName: "SLVR", color: "#a7a9ab" },
  "BLUE": { name: "Blue Line", shortName: "BLUE", color: "#5785b1" },
  "RED": { name: "Red Line", shortName: "RED", color: "#e40d17" },
};
...
// Main function
async function main() {
  console.log("🏗️ Parsing Pittsburgh GTFS data for T Light Rail...\n");

  // 1. Read trips.txt to get shape_id -> route_id mapping for T lines only
  ...
  console.log(`Found ${shapeToRoute.size} unique shapes for T routes`);
  for (const [routeId, shapes] of routeShapes) {
    console.log(`  ${T_ROUTES[routeId].shortName}: ${shapes.size} shapes`);
  }
  ...
  for (const [routeId, routeInfo] of Object.entries(T_ROUTES)) {
    const shapes = routeShapes.get(routeId);
    if (!shapes) {
      console.log(`  ${routeInfo.shortName}: No shapes found`);
      continue;
    }
  ...
  for (const trip of trips) {
    if (T_ROUTES[trip.route_id]) {
      tripToRoute.set(trip.trip_id, T_ROUTES[trip.route_id].shortName);
    }
  }
  ...
  for (const stop of stops) {
    ...
    if (!routes) continue; // Skip stops not on T lines
  ...
  console.log(`Found ${stopFeatures.length} T stations`);
  ...
  const routesPath = path.join(OUTPUT_DIR, "pittsburghTRoutes.json");
  fs.writeFileSync(routesPath, JSON.stringify(routesGeoJSON, null, 2));
  console.log(`  Wrote ${routesPath}`);
  
  const stopsPath = path.join(OUTPUT_DIR, "pittsburghTStops.json");
