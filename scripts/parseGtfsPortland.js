...
function main() {
  ...
  console.log(
    `Found ${railRoutes.length} rail routes:`,
    railRoutes.map((r) => `${r.route_id} (${r.route_long_name})`),
  );
    ...
    if (isStreetcar) {
      ...
      shapes.forEach(([shapeId, info]) => {
        if (info.count >= 10) {
          ...
          console.log(
            `${data.route_id} dir ${data.direction_id}: included shape ${shapeId} (${info.headsign || "no headsign"}, ${info.count} trips)`,
          );
        }
      });
    } else {
